...
dams_snapped_joined <- dams_snapped %>%
  mutate(cluster =
           st_equals(dams_snapped, dams_snapped) %>%
...
ggplot() +
  ...
  ggspatial::layer_spatial(shape_basin, fill = NA, color = "gray90") +
  ggspatial::layer_spatial(shape_river, aes(color = log10(UPLAND_SKM)) )+
  ggspatial::layer_spatial(dams_snapped,color = "blue") +
  ggspatial::layer_spatial(dams_snapped_joined,color = "black") +
  scale_color_viridis(direction = -1, name= "upstream area \n(log10[Km^2])") +
  theme(legend.position = "bottom") +
  ggspatial::annotation_scale(location = "bl", style = "ticks") +
  ggspatial::annotation_north_arrow(location = "br")
